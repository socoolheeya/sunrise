...
현재 FastAPI 구현은 model artifact의 `trained_at`, backtest metric, drift baseline을 `model-status`에서 노출하고, score 조건 오디언스가 같은 prediction artifact를 사용하도록 연결한다. 아직 tenant별 calibration과 실제 feature distribution drift 계산은 별도 feature store/read model 단계가 필요하다.
...
| `GET /v1/analytics/attribution` | campaign touchpoint 이후 구매를 last-touch 기준으로 채널별 집계 |
...
| 유입/어트리뷰션 | channel/source/medium/campaign 기반 세션·구매·매출 API, last-touch attribution API | ClickHouse 사전집계 + first/non-direct attribution rule 확장 |
| 숨은 매출 | 총 매출, 온사이트 매출, 기여 매출, tracking loss breakdown API | 정산/주문 SSOT와 reconciliation |
| DataTalk | 일일 사이트 프로파일링 API, frozen snapshot 저장 API | scheduled batch, 시장 비교, 알림톡/이메일 발송 |
| AI | 사이트 진단, 캠페인 제안, 카피 생성 | LLM/agent tool 연동과 review workflow |
| Prediction | ML artifact 기반 purchase/churn/affinity scoring, explain, model-status drift baseline | BG/NBD/Pareto/NBD/Gamma-Gamma CLV, tenant calibration, 실측 drift monitoring, batch feature store |
...
| Audience | 50+ 템플릿 catalog, cached preview, prediction score 조건, materialization API | 조건 DSL 편집기, scheduled refresh, 대용량 membership store |
